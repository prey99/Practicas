...
treasure1 = {'rope': 1, 'torch': 0, 'gold coin': 42, 'dagger': 1, 'arrow': 12}
treasure2 = {'rope': 1}
...
print('\n')
def displayInventory(myThings, zeroListed = True):
    '''
    Parameters
    ----------
    myThings : Dictionary data type with format item:qtty.
    zeroListed : Printing zero qtty items.
                 Use False to avoid.
    Returns
    -------
    None.
    ...
    for ITEM, QTTY in myThings.items() :  
        # Find incorrect data type in keys or values.  
        if type(ITEM) == str and type(QTTY) == int :
            # Check if zeroListed flag is present.
            if zeroListed == False and QTTY == 0:
                warning = '\033[1;30;43m(Note: some items were not printed\nbecause quantity was not an integer > 0\and parameter zeroListed = False.)\033[0m'
            else :
                print(myThings[ITEM], ITEM)
                totalItems += QTTY                
        else:
            warning = '\033[2;30;41m(Note: some items were not printed\nbecause names were not strings,\nor quantity was not an integer.)\033[0m'
...
# print('******treasure4 avoiding zero qtty. items.******'); displayInventory(treasure4, False)
